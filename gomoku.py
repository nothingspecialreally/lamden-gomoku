...
@export
def play(game_id=int, column: int, row: int, player_id: int): # janky as shit, should use lookup (but I'm lazy)
    ...
    assert game_state[game_id][column][row] == None, 'Position already occupied!'
...
@export
def call_game_default(game_id=int, player_id: int):
...
def check_win(game_id=int, player_id: int, possible_positions: list):
    count = 0
    for position in list(possible_positions):
